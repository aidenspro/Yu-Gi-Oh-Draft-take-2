...
function draftDisplay() {
  ...
  const handleOnClick = (infoArray) => {
    if (!draftOver) {
      if (
        infoArray[3].split(' ')[0] != 'Effect' ||
        infoArray[3].split(' ')[0] != 'Normal' ||
        infoArray[3].split(' ')[0] != 'Flip' ||
        infoArray[3].split(' ')[0] != 'Tuner'
      ) {
  ...
  return (
    <div className="topdiv">
      ...
      <div className="draftheaders">
        <h4 className="">{'Monsters'}</h4>
        <h4 className="">{'Spells'}</h4>
        <h4 className="counter">
          Total{'\n'}
          {num == 1 || num == 0 ? '0' : num - 1}{' '}
        </h4>
        <h4 className="">{'Traps'}</h4>
        <h4 className="">{'Extra Deck'}</h4>
      </div>
